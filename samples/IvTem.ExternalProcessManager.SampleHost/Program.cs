...
internal static class SampleWorkerPath
{
    ...
    public static string Resolve()
    {
        ...
        string configurationName = outputDirectory.Parent?.Name ?? "Debug";
        string samplesDirectory = Path.GetFullPath(Path.Combine(
            AppContext.BaseDirectory,
            "..",
            "..",
            "..",
        ...
        if (File.Exists(workerPath) == false)
            throw new FileNotFoundException("Build the sample worker before running the sample host.", workerPath);
